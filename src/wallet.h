...
enum WalletFeature
{
    ...
    FEATURE_MASTERNODES = 61000, // masternodes
...
enum AvailableCoinsType
{
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 BTA at the same time
...
{
private:
    bool SelectCoins(const CAmount& nTargetValue, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl *coinControl = NULL, AvailableCoinsType nCoinType=ALL_COINS, bool fUseIX = true) const;
...
// orig    bool SelectCoins(const CAmount& nTargetValue, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl *coinControl = NULL) const;
...
public:
    ...
    /// Get 10000 BTA output and keys which can be used for the Masternode
    ...
    bool CreateTransaction(const std::vector<std::pair<CScript, CAmount> >& vecSend,
                           CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl *coinControl = NULL, AvailableCoinsType nCoinType=ALL_COINS, bool fUseIX=false, CAmount nFeePay=0);
    bool CreateTransaction(CScript scriptPubKey, const CAmount& nValue,
                           CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl *coinControl = NULL, AvailableCoinsType nCoinYype=ALL_COINS, bool fUseIX=false, CAmount nFeePay=0);
    ...
    bool GetBudgetSystemCollateralTX(CTransaction& tx, uint256 hash, bool fUseIX);
    bool GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool fUseIX);
