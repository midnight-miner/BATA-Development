...
//
// Bootup the Masternode, look for a 10000 Bata input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if (status != ACTIVE_MASTERNODE_STARTED) {
        ...
        if (Params().NetworkID() == CBaseChainParams::MAIN) {
            if (service.GetPort() != 5784) {
                notCapableReason = strprintf("Invalid port: %u - only 5784 is supported on mainnet.", service.GetPort());
                LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
                return;
            }
        } else if (service.GetPort() != 33813) {
            notCapableReason = strprintf("Invalid port: %u - only 33813 is supported on testnet.", service.GetPort());
            LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
            return;
        }
...
bool CActiveMasternode::CreateBroadcast(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& errorMessage, CMasternodeBroadcast &mnb, bool fOffline) {
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN) {
        if (service.GetPort() != 5784) {
            errorMessage = strprintf("Invalid port %u for masternode %s - only 5784 is supported on mainnet.", service.GetPort(), strService);
            LogPrintf("CActiveMasternode::CreateBroadcast() - %s\n", errorMessage);
            return false;
        }
    } else if (service.GetPort() != 33813) {
        errorMessage = strprintf("Invalid port %u for masternode %s - only 33813 is supported on testnet.", service.GetPort(), strService);
        LogPrintf("CActiveMasternode::CreateBroadcast() - %s\n", errorMessage);
        return false;
    }
...
bool CActiveMasternode::GetMasterNodeVin(CTxIn& vin, CPubKey& pubkey, CKey& secretKey, std::string strTxHash, std::string strOutputIndex) {
    ...
    if (!strTxHash.empty()) {
        ...
        int outputIndex;
        try {
            outputIndex = atoi(strOutputIndex.c_str());
...
vector<COutput> CActiveMasternode::SelectCoinsMasternode()
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        if (out.tx->vout[out.i].nValue == Params().MasternodeAmount()) { //exactly
