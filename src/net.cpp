    ...
    if (fListen && pnode->fSuccessfullyConnected) {
        ...
        if (IsPeerAddrLocalGood(pnode)
                && (!addrLocal.IsRoutable()
                        || GetRand(
                                (GetnScore(addrLocal) > LOCAL_MANUAL) ? 8 : 2)
                                == 0)) {
...
CNode*
FindNode(const std::string& addrName) {
    LOCK(cs_vNodes);
    BOOST_FOREACH(CNode * pnode, vNodes)
...
CNode*
ConnectNode(CAddress addrConnect, const char *pszDest, bool obfuScationMaster) {
    ...
    if (pszDest ?
            ConnectSocketByName(addrConnect, hSocket, pszDest,
        ...
        // Add node
        CNode* pnode = new CNode(hSocket, addrConnect, pszDest ? pszDest : "",
                false);
        pnode->AddRef();
        ...
        if (obfuScationMaster)
            pnode->fObfuScationMaster = true;
...
void CNode::PushVersion() {
    ...
    PushMessage("version", PROTOCOL_VERSION, nLocalServices, nTime, addrYou,
            addrMe, nLocalHostNonce,
...
void CNode::copyStats(CNodeStats &stats) {
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Bata users should be well used to small numbers with many decimal places by now :)
...
void SocketSendData(CNode *pnode) {
    ...
    while (it != pnode->vSendMsg.end()) {
        ...
        if (nBytes > 0) //nBytes bigger than 0
            ...
            if (nBytes < 0)   //nBytes smaller than 0
            {
                int nErr = WSAGetLastError();
                if (nErr != WSAEWOULDBLOCK && nErr != WSAEMSGSIZE
                        && nErr != WSAEINTR && nErr != WSAEINPROGRESS) {
                    if ((nErr != WSAEMSGSIZE) && (nErr != WSAEMSGSIZE)
                            && (nErr != WSAEINTR) && (nErr != WSAEINPROGRESS)) {
...
void ThreadDNSAddressSeed() {
    ...
    if ((addrman.size() > 4) && (!GetBoolArg("-forcednsseed", false))) {
        MilliSleep(11 * 1000);
...
void ThreadOpenAddedConnections() {
    ...
    for (unsigned int i = 0; true; i++) {
        ...
        BOOST_FOREACH(string & strAddNode, lAddresses)
        {
            ...
            if (Lookup(strAddNode.c_str(), vservNode, Params().GetDefaultPort(),
                    fNameLookup, 0)) {
...
bool OpenNetworkConnection(const CAddress& addrConnect,
        CSemaphoreGrant *grantOutbound, const char *pszDest, bool fOneShot) {
    //
            ...
            // Send messages
            {
                ...
                if (lockSend)
                    g_signals.SendMessages(pnode,
                            pnode == pnodeTrickle || pnode->fWhitelisted);
            }
            boost::this_thread::interruption_point();
        }
...
void StartNode(boost::thread_group& threadGroup) {
    ...
    if (!GetBoolArg("-dnsseed", true))
        ...
        threadGroup.create_thread(
                boost::bind(&TraceThread<void (*)()>, "dnsseed",
    ...
    threadGroup.create_thread(
            boost::bind(&TraceThread<void (*)()>, "addcon",
                    &ThreadOpenAddedConnections));

    // Initiate outbound connections
    threadGroup.create_thread(
            boost::bind(&TraceThread<void (*)()>, "opencon",
                    &ThreadOpenConnections));

    // Process messages
    threadGroup.create_thread(
            boost::bind(&TraceThread<void (*)()>, "msghand",
                    &ThreadMessageHandler));

    // Dump network addresses
    threadGroup.create_thread(
            boost::bind(&LoopForever<void (*)()>, "dumpaddr", &DumpAddresses,
...
CNode::CNode(SOCKET hSocketIn, CAddress addrIn, std::string addrNameIn,
        bool fInboundIn) :
...
void CNode::EndMessage() UNLOCK_FUNCTION(cs_vSend)
{
    ...
    if (mapArgs.count("-dropmessagestest")
            && GetRand(GetArg("-dropmessagestest", 2)) == 0) {
    ...
    memcpy((char*) &ssSend[CMessageHeader::MESSAGE_SIZE_OFFSET], &nSize,
            sizeof(nSize));
    ...
    memcpy((char*) &ssSend[CMessageHeader::CHECKSUM_OFFSET], &nChecksum,
            sizeof(nChecksum));

    LogPrint("net", "(%d bytes) peer=%d\n", nSize, id);

    std::deque<CSerializeData>::iterator it = vSendMsg.insert(vSendMsg.end(),
            CSerializeData());
