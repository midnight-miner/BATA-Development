...
{
    // Amount of blocks in a months period of time (using 1.5 minutes per) = (960*30)
...
void CBudgetManager::SubmitFinalBudget()
{
    ...
    // allow submitting final budget only when 2 days left before payments
...
bool CBudgetManager::HasNextFinalizedBudget()
{
    ...
    if (nBlockStart - pindexPrev->nHeight > 960 * 2) return true; //we wouldn't have the budget yet
...
CAmount CBudgetManager::GetTotalBudget(int nHeight)
{
    ...
    CAmount nSubsidy = 0;
    if ( (nHeight <= Params().LAST_POW_BLOCK()) && (nHeight >= Params().FIRST_MN_BLOCK()) ) {
        nSubsidy = 0.25 * COIN;
    } else {
        nSubsidy = 0 * COIN;
    }

    // Amount of blocks in a months period of time (using 1.5 minutes per) = (60*24*30)/1.5
    if(Params().NetworkID() == CBaseChainParams::MAIN) return ((nSubsidy/100)*10)*960*30;
...
void CBudgetManager::NewBlock()
{
    ...
    if (masternodeSync.IsSynced()) {
        ...
        if (chainActive.Height() % 1000 == rand() % 1000) {
            ClearSeen();
            ResetSync();
        }
