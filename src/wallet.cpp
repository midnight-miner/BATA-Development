        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if(nCoinType == ONLY_DENOMINATED) {
                    ...
                    found = !(fMasterNode && pcoin->vout[i].nValue == Params().MasternodeAmount());
                } else if(nCoinType == ONLY_NONDENOMINATED_NOT10000IFMN) {
                    if (IsCollateralAmount(pcoin->vout[i].nValue)) continue; // do not use collateral amounts
                    found = !IsDenominatedAmount(pcoin->vout[i].nValue);
                    if(found && fMasterNode) found = pcoin->vout[i].nValue != Params().MasternodeAmount(); // do not use Hot MN funds
                } else if(nCoinType == ONLY_10000) {
                    found = pcoin->vout[i].nValue == Params().MasternodeAmount();
                } else {
                    found = true;
                }
                if(!found) continue;
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType nCoinType, bool fUseIX) const
{
    // Note: this function should never be used for "always free" tx types like dstx

    vector<COutput> vCoins;
    AvailableCoins(vCoins, true, coinControl, nCoinType, fUseIX);
    ...
    //if we're doing only denominated, we need to round up to the nearest 0.1 BTA
    if(nCoinType == ONLY_DENOMINATED) {
        ...
        BOOST_FOREACH(int64_t v, obfuScationDenominations)
        {
            BOOST_FOREACH(const COutput& out, vCoins)
            {
                if(out.tx->vout[out.i].nValue == v                                            //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1*COIN)+100 //round the amount up to .1 BTA over
                ){
                    CTxIn vin = CTxIn(out.tx->GetHash(),out.i);
                    int rounds = GetInputObfuscationRounds(vin);
                    // make sure it's actually anonymized
...
bool CWallet::SelectCoinsByDenominations(int nDenom, int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, int64_t& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax)
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(nValueRet + out.tx->vout[out.i].nValue <= nValueMax){
            ...
            //
            // bit 0 - 10000 BTA+1 ( bit on if present )
            // bit 1 - 1000 BTA+1
            // bit 2 - 100 BTA+1
            // bit 3 - 10 BTA+1
            // bit 4 - 1 BTA+1
...
bool CWallet::GetBudgetSystemCollateralTX(CTransaction& tx, uint256 hash, bool fUseIX)
{
    CWalletTx wtx;
    if(GetBudgetSystemCollateralTX(wtx, hash, fUseIX)){
        tx = (CTransaction)wtx;
        return true;
    }
    return false;
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool fUseIX)
{
    // make our change address
    ...
    CCoinControl *coinControl=NULL;
    bool success = CreateTransaction(vecSend, tx, reservekey, nFeeRet, strFail, coinControl, ALL_COINS, fUseIX, (CAmount)0);
    if(!success){
...
bool CWallet::CreateTransaction(const vector<pair<CScript, CAmount> >& vecSend,
                                CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, AvailableCoinsType nCoinType, bool fUseIX, CAmount nFeePay)
{
    if(fUseIX && nFeePay < CENT) nFeePay = CENT;
...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, nCoinType, fUseIX))
                {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-bata-address
                    CScript scriptChange;
                                ...
                                CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, AvailableCoinsType nCoinType, bool fUseIX, CAmount nFeePay)
{
    ...
    return CreateTransaction(vecSend, wtxNew, reservekey, nFeeRet, strFailReason, coinControl, nCoinType, fUseIX, nFeePay);
}
